		...
		<div class="">
			@if($tor->idConfederacion === NULL)
				...
				@foreach($partidos as $part)
					...
					@if($part->idTorneo === $tor->idTorneo && $part->fechaPartido == $nuevaFecha)
					<div class="">
						<div class="row justify-content-center" >
...
<div>
	...
	<?php
	$fechaSig = date("m-d",strtotime($nuevaFecha."+ 1 days"));
    $fechaAnt = date("m-d",strtotime($nuevaFecha."- 1 days"));
	?>

	<div >
		<a href="/partido/{{$nuevaFecha}}/fechasAtras" class="btn btn-warning"><span class="glyphicon glyphicon-wrench">Dia Anterior {{$fechaAnt}}</span></a>
		<a href="/partido/{{$nuevaFecha}}/fechas" class="btn btn-warning"><span class="glyphicon glyphicon-wrench">Dia Siguiente {{$fechaSig }}</span></a>
	</div>
</div>
